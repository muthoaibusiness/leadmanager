...
function AgentActivityRow({ agent, leads, acts }) {
  ...
  return (
    <div className="aa-row">
      <div className="aa-av" style={{ background: c }}>{ini(agent.name)}</div>
      ...
      <div className="aa-last">{recentAct ? <><div className="aa-act">{recentAct.description}</div><div className="aa-time">{fmtAgo(recentAct.timestamp)}</div></> : <div className="aa-time">No activity</div>}</div>
    </div>
  );
}
...
export default function ManagementDash() {
  const { dbVersion, setView, setTeamFilter, setAgentFilter, setTab, setSearch } = useApp();
        ...
        <>
          ...
          <div className="grid-2" style={{ marginBottom: '24px' }}>
            {db.users.filter(u => u.role === ROLES.TL).map(tl => {
              const tLeads = allLeads.filter(l => l.teamId === tl.teamId);
      ...
      {activeTab === 1 && (
        <>
          ...
          <div className="analytics-card" style={{ marginBottom: '16px' }}>
            ...
            {allAgents.map(agent => (
              <AgentActivityRow key={agent.id} agent={agent} leads={allLeads} acts={allActs} />
            ))}
          </div>
        </>
      )}
          ...
          <div className="analytics-card">
            ...
            {deletionLog.length > 0 && (
              <div className="del-log">
                <div className="dl-hdr"><div>Lead</div><div>Status at Deletion</div><div>Deleted By</div><div>When</div></div>
                {deletionLog.map((d, i) => (
